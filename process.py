...
class ProcessDgreData:
    def __init__(self):
        ...
        self.train_file = self.data_path + "ori_data/60_output.json"
    ...
    def get_ner_data(self):
        ...
        for did, d in enumerate(data):
            ...
            for dist in d['annotations'][0]['result']:
                if dist['type']=='labels':
                    start = dist['value']['start']
                    end = dist['value']['end']
                    B_str = "B-"+dist['value']['labels'][0]
                    I_str = "I-"+dist['value']['labels'][0]
                    tmp["labels"][start] = B_str
        ...
        labels = ["EVE", "CAR","PEO","LOC","JUD","MES","ACT","OBJ"]
    ...
    def get_re_data(self):
        ...
        for did, d in enumerate(tqdm(data)):
            d = json.loads(d)
            text = d['data']['text']
            for rel_id, dist in enumerate(d['annotations'][0]['result']):
                if dist['type']=='labels': 
                    id = dist['id']
                    index[id]=dist['value']['labels'][0]
                elif dist['type']=='relation':
                    tmp = {}
                    tmp['text'] = text
                    tmp["labels"] = []
                    h_name = index[dist['from_id']]
                    t_name = index[dist['to_id']]
                    relation = dist['labels'][0]
