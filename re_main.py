    ...
    def train(self):
        ...
        for epoch in range(1, self.epochs + 1):
            for step, batch_data in enumerate(self.train_loader):
                ...
                input_ids = batch_data["input_ids"].to(self.device).long()
                attention_mask = batch_data["attention_mask"].to(self.device).long()
                token_type_ids = batch_data["token_type_ids"].to(self.device).long()
                labels = batch_data["labels"].to(self.device).long()
...
def main(data_name):
    ...
    with open(os.path.join(args.output_dir, "re_args.json"), "w", encoding="utf-8") as fp:
        json.dump(vars(args), fp, ensure_ascii=False, indent=2)

    tokenizer = BertTokenizer.from_pretrained(args.bert_dir)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    with open(os.path.join(args.data_path, "train.txt"), "r", encoding="utf-8", errors='ignore') as fp:
        train_data = fp.read().split("\n")
    train_data = [json.loads(d) for d in train_data]

    with open(os.path.join(args.data_path, "dev.txt"), "r", encoding="utf-8", errors='ignore') as fp:
